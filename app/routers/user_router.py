...
async def register_user(request: Request, user_data: user_models.RegisterRequest):
   try:
      user_id = await user_manager.add_user(login=user_data.login, password=user_data.password)
      if user_id:
         return user_models.RegisterSuccessResponse(status="success", message="User registered successfully")
      else:
